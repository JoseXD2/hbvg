...
class PuppetJumpscare extends MusicBeatSubstate
{
	...
	override public function create():Void {
		...
		jumpscare.frames = Paths.getSparrowAtlas('jumpscares/jump_puppet','shared');
		...
		new FlxTimer().start(0.1, function(jump:FlxTimer)
		{
			...
			FlxG.sound.play(Paths.sound('puppet_jump', 'shared'));
			...
			new FlxTimer().start(1.5, function(jump:FlxTimer)
			{
				...
				new FlxTimer().start(3, function(tmr:FlxTimer)
				{
					...
					FlxG.sound.play(Paths.sound('puppetbox', 'shared'));
					
					staticeffect.alpha = 0.05;
